...
var lastX1 = -1, lastY1 = -1, lastX2 = -1, lastY2 = -1; var x1 = -1, y1 = -1, x2 = -1, y2 = -1; var touch1 = null, touch2 = null; var factor = 200.0 / textCanvas.height; var scaleFactor = 2.0; function phoneKeyDown(event) {
    isKeyTap = true; if (event.targetTouches.length == 1) { touch1 = event.targetTouches[0]; x1 = touch1.clientX - offsetLeft, y1 = touch1.clientY - offsetTop; lastX1 = x1, lastY1 = y1; }
...
function phoneKeyMove(event) {
    if (event.targetTouches.length == 1) {
        touch1 = event.targetTouches[0]; x1 = touch1.clientX - offsetLeft, y1 = touch1.clientY - offsetTop; if (isPhone) { if (isMove) { if (objectIndex != -1) { if (Math.abs(x1 - lastX1) > 2 || Math.abs(y1 - lastY1) > 2) { glRunTime.objectMove(objectIndex, 2 * (x1 - lastX1), -2 * (y1 - lastY1)); lastX1 = x1, lastY1 = y1; return; } } } }
        ...
        touch1 = event.targetTouches[0], touch2 = event.targetTouches[1]; x1 = touch1.clientX - offsetLeft, y1 = touch1.clientY - offsetTop; x2 = touch2.clientX - offsetLeft, y2 = touch2.clientY - offsetTop; let vecX1 = lastX1 - lastX2, vecY1 = lastY1 - lastY2; let vecX2 = x1 - x2, vecY2 = y1 - y2; let mVec1 = Math.sqrt(vecX1 * vecX1 + vecY1 * vecY1); let mVec2 = Math.sqrt(vecX2 * vecX2 + vecY2 * vecY2); let seta1 = Math.atan2(lastY2 - lastY1, lastX2 - lastX1); let seta2 = Math.atan2(y2 - y1, x2 - x1); if (Math.abs(seta1 - seta2) <= 0.01 && Math.abs(mVec2 - mVec1) <= 5.0) { if (Math.abs(x1 - lastX1) > 5 || Math.abs(y1 - lastY1) > 5) { glRunTime.move(2 * (x1 - lastX1), -2 * (y1 - lastY1)); } } else {
            let dist1 = Math.pow(Math.pow(lastX1 - lastX2, 2) + Math.pow(lastY1 - lastY2, 2), 0.5); let dist2 = Math.pow(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2), 0.5); let scale = 0; if (dist2 / dist1 > 1.0) { scale = dist2 / dist1 * scaleFactor; } else { scale = dist2 / dist1 / scaleFactor; }
...
var dragLeft = false, dragMid = false, dragRight = false; var lastX = -1, lastY = -1; function webKeyDown(event, textCanvas) {
    if (isLockCavans) { return; }
    isKeyTap = true; let rect = event.target.getBoundingClientRect(); let x = event.clientX - offsetLeft, y = event.clientY - offsetTop; if (rect.left <= x && x < rect.right && rect.top <= y && y < rect.bottom) {
        lastX = x, lastY = y; switch (event.button) {
            case 0: dragLeft = true; textCanvas.oncontextmenu = function () { return false; }
...
function webKeyUp(event, textCanvas) {
    if (isLockCavans) { return; }
    switch (event.button) {
        case 0: dragLeft = false; if (isDigitalTwinMode) { return; }
            if (isKeyTap) {
                if (!isShiftDown) {
                    objectIndex = glRunTime.pick(lastX, lastY, true, false); pickObjectVisible = glRunTime.getObjectVisible(objectIndex); pickObjectTransparent = glRunTime.getObjectTransparent(objectIndex); curDate = new Date(); let curTime = curDate.getTime(); if (curTime - lastTime < 500 && lastObjectIndex == objectIndex) { setFocusOnModel(); }
                    lastTime = curTime; lastObjectIndex = objectIndex;
                } else { objectIndex = glRunTime.pick(lastX, lastY, true, true); }
...
function webKeyMove(event, textCanvas) {
    if (isLockCavans) { return; }
    let x = event.clientX - offsetLeft, y = event.clientY - offsetTop; if (isKeyTap) {
        if ((!isShiftDown) && dragMid) { let factor = 200.0 / textCanvas.height; let degreeX = factor * (x - lastX); let degreeY = factor * (y - lastY); glRunTime.rotate(degreeX, degreeY, 0); }
    ...
    } else { glRunTime.pickAnnotation(x, y, false); }
    lastX = x, lastY = y;
}
function addMouseListener(textCanvas) {
    textCanvas.ontouchstart = function (event) { phoneKeyDown(event); }
...
window.onresize = canvasOnResize; document.onmousewheel = function (event) {
    if (event.preventDefault) { event.preventDefault(); event.stopPropagation(); } else { event.cancelBubble = true; event.returnValue = false; }
    return false;
}
function moveModel() {
    if (isPhone) { if (isMove) { isMove = false; } else { isMove = true; } }
    isMove = true;
}
...
function setAnimationStart() {
    uTotalFrame = glRunTime.getTotalFrame(); isLockCavans = true; if (animationStatus == ANIMTERMINAL) { setHome(); }
...
function animPause() {
    if (animationClock != null) { clearTimeout(animationClock); }
    animationClock = null; isLockCavans = false; animationStatus = ANIMPAUSE;
}
function animTerminal() {
    uCurFrame = 0; getCurFrame(uCurFrame); if (glRunTime.setCameraAnim(uCurFrame)) { glRunTime.setObjectAnim(uCurFrame); glRunTime.setAnnotationAnim(uCurFrame); }
    animPause(); animationStatus = ANIMTERMINAL;
}
function setCurFrame(frame) {
    if (frame >= 0) {
        uCurFrame = frame; if (glRunTime.setCameraAnim(uCurFrame)) { glRunTime.setObjectAnim(uCurFrame); glRunTime.setAnnotationAnim(uCurFrame); }
...
function pickModelByIndex(indexs) {
    objectIndex = glRunTime.pickModelByIndexs(indexs); pickObjectIndexs = glRunTime.getPickObjectIndexs(); if (indexs.length == 1) { pickObjectVisible = glRunTime.getObjectVisible(indexs[0]); pickObjectTransparent = glRunTime.getObjectTransparent(indexs[0]); }
...
function canvasOnResize() {
    gl.canvas.width = gl.canvas.clientWidth; gl.canvas.height = gl.canvas.clientHeight; if (initRenderFlag) { glRunTime.resetWindow(gl.canvas.clientWidth, gl.canvas.clientHeight); }
...
var g_nAnimationStart = 0; var g_nAnimationEnd = 0; function PlaySceneAnimation() {
    isLockCavans = true; if (animationStatus == ANIMTERMINAL) { setHome(); }
