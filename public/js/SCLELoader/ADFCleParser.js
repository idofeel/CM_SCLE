...
function ADFCleParser() {
    ...
    this.CleStreamModelDataParser = function (data) {
       ...
       // 读取三角面片数据
        if (data._uIsUV  == 1) {
            nCount = sizeByte / 4;
            data._arrVertexData = new Float32Array(nCount);
            for (let i = 0; i < nCount; i+=8) {
                data._arrVertexData[i] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+1] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+2] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+3] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+4] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+5] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+6] = this.parseStreamADF_FLOAT();
                data._arrVertexData[i+7] = this.parseStreamADF_FLOAT();
            }
        }
        else {
            nCount = sizeByte / 4;
    ...
    this.parseLicData = function () {
        if (!g_licData || g_licData.length == 0) {
            return;
        }
    ...
    this.parseMain = function (data) {
        ...
        if (!g_strCopy || g_strCopy.length == 0) {
            return;
        }
