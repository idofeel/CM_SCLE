...
function ADF_MODEL_SAVEDATA() {    
    this._arrVertexPosition = null;                                 // 顶点数据(每个顶点包含8个Float32(3个位置+3个法矢向量+2纹理坐标)), Float32Array类型
    ...
    this._arrIndexData = null;                                      // 索引组，组内3个元素为一组(三角片)或2个元素为一组(线段，Uint32数组结构, Uint32Array类型
                                                                    // 顶点数量小于65535, 把顶点索引变为Uint16Array类型数组，减少索引值
    ...
    this._uIsUV = 1;			                                    // 版本3，增加是否存在UV坐标，0表示不存在，1表示存在，默认值为1
                                                                    // UV坐标不能存在时， 顶点数据(每个顶点包含6个Float32(3个位置+3个法矢向量)), Float32Array类型
    this._arrMdlPtIndex = null;                                     // 增加剖切时需要的顶点索引对照关系
...
function CleStreamResFileInfo() {  
    ...
    this._imageWidth = 0;                   // 图像宽度，Uint32
    this._imageHeight = 0;                  // 图像高度，Uint32
    this._imageData = null;                 // 图像数据，长度为_size
}
...
function ADF_COMMENTPROPERTY() {
    ...
    this.Clone = function () {
        ...
        newData._stuCamera.Copy(this._stuCamera);         
        newData._uStartFrameID = this._uStartFrameID;
        newData._uFrameSize = this._uFrameSize;	
    ...
    this.Copy = function (data) {  
        ...
        this._stuCamera.Copy(data._stuCamera);         
        this._uStartFrameID = data._uStartFrameID;
        this._uFrameSize = data._uFrameSize;  
 	}	
}
...
function ADF_AnnotationGraphicData() {
    ...
    this.Clone = function () {
        ...
        for (var i in this.facetIndexes){
            newData.facetIndexes[i] = this.facetIndexes[i];
        }
    ...
    this.Copy = function (data) {
        ...
        this.facetIndexes.splice(0, this.facetIndexes.length);
        for (var i in data.facetIndexes){
            this.facetIndexes[i] = data.facetIndexes[i];
        } 
...
function ADF_Annotation() {
    ...
    this._Loaded = false;                               // 是否已经加载了标注项和视图集
