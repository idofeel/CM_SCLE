...
const STYLE_DARK_YELLOW = "#ffd700";
const DOT_RIDUS = 3;
...
function Canvas2D() {
    ...
    this.draw = function() {
        ...
        if (this.isShowUsrComment) {
            ...
            if (this.isDuringComment) {
                this.drawDuringUsrAnnoation(g_sceneAnnotation.usrNewComment, this.mousePt.x, this.mousePt.y);
            }
        ...
        if (this.isDuringMeasure) {
            this.drawDuringMeasure(g_sceneMeasure.pickUnitFirst, g_sceneMeasure.pickLeaderPos, this.mousePt.x, this.mousePt.y);
        }
    ...
    this.drawTestMode = function() {
        ...
        gl2d.fillStyle = STYLE_DARK_YELLOW;
        gl2d.strokeStyle = STYLE_DARK_YELLOW;
        // 绘制测试模式
        if (g_nEventVersion >=2) {
            gl2d.fillText(g_strCopy, 5, this.windowHeight - 5);
        }
    }
    ...
    this.addComment = function(comment) {
        ...
        this.m_arrAnnotRect.push(this.calAnnotRect(comment.stuAnnot));
        this.m_arrAnnotVisiable.push(true);
    }
    ...
    /**
     * @description 将物件坐标转换为屏幕坐标
     * @param {*} modelPos 
     * @returns {*}  屏幕坐标 -1 ~ 1
     */
    this.cvtModelToScreen = function(modelPos, objIdx) {
        var newPt = new Point3(0, 0, 0); // -1 ~ 1
        var screenPos = {x: 0, y: 0}
        var objIdx = objIdx  == -1 ? objectIndex : objIdx
        mat4.multiply(this.MVPMatrix, g_webgl.PVMattrix, g_glprogram.getObjectModelMatrix(objIdx));
        CalTranslatePoint(modelPos.x, modelPos.y, modelPos.z,  this.MVPMatrix, newPt);

        // 如果坐标在摄像机剪裁面之外
        if (newPt.z < 0.0 || newPt.z > 1.0) {
            return screenPos
        } else {
            screenPos.x = newPt.x
            screenPos.y = newPt.y
